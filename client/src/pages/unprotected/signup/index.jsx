    ...
    return (
        <>
            ...
            <main className='grid min-h-screen grid-cols-1 md:grid-cols-[3fr_2fr] lg:grid-cols-[2fr_3fr]'>
                <article className='bg-color3-200 grid place-items-center p-4 md:p-12'>
                    ...
                    {mode === null ? (
                        <article className='w-full max-w-md'>
                            <h2 className='font-primary text-color1-100 text-6xl font-black'>Olá!</h2>
                            <p className='text-color1-400 mb-8 text-xl text-balance'>
                                Para criar uma conta é preciso especificar o tipo da conta. Que tipo de conta deseja?
                            </p>
                            <div className='flex flex-wrap gap-4'> {/* Talvez mudar as cores */}
                                <button
                                    className='bg-color1-100 flex-1 basis-50 hover:shadow-lg-hard flex min-h-12 w-full cursor-pointer items-center justify-center gap-4 rounded-xl py-4 transition-all duration-150'
                                    onClick={() => setMode('Teacher')}
                                >
                                    <IconSchool size={26} className='stroke-color3-200' />
                                    <span className='text-color3-200 text-lg font-semibold'>Aluno</span>
                                </button>
                                <button
                                    className='bg-color1-100 flex-1 basis-50 hover:shadow-lg-hard flex min-h-12 w-full cursor-pointer items-center justify-center gap-4 rounded-xl py-4 transition-all duration-150'
                                    onClick={() => setMode('Student')}
                                >
                                    <IconChalkboardTeacher size={26} className='stroke-color3-200' />
                                    <span className='text-color3-200 text-lg font-semibold'>Professor</span>
                                </button>
                            </div>
                        </article>
                    ) : (
